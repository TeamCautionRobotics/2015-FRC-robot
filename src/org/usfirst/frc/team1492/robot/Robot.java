...
public class Robot extends SampleRobot {
	...
	Joystick stickRight;
	Joystick stickAux;
	ArrayList<Joystick> joystickList = new ArrayList<Joystick>();
	...
	public void operatorControl() {
		...
		while (isOperatorControl() && isEnabled()) {
			...
			if (!recordingTape && SmartDashboard.getBoolean("Record", false)) {
				tapeTick = 0;
				currentTape.currentActionIndex = 0;
			}
			if(recordingTape && !SmartDashboard.getBoolean("Record", false)){
				saveTape(currentTape, "tape1");
			}
	...
	private void saveTape(Tape tape, String name) {
		String fileString = "";
	...
	private Tape loadTape(String name) {
		Tape tape = new Tape();
	...
	public void test() {
		...
		Tape tempTape = loadTape("tape1");
		if(tempTape.actionList.size() > 0){
			currentTape = tempTape;
		}
	...
	public double getAxis(Joystick stick, AxisType axis){
		int stickIndex = joystickList.indexOf(stick);
		if(playingTape){
			return currentTape.currentState.getJoystick(stickIndex).axes[axis.value];
		} else if(recordingTape){
			if(currentTape.currentState.getJoystick(stickIndex).axes[axis.value] != stick.getAxis(axis)){
				currentTape.addAction(new ActionAxis(tapeTick, stickIndex, axis.value, stick.getAxis(axis)));
			}
		}
	...
	public boolean getButton(Joystick stick, int index){
		int stickIndex = joystickList.indexOf(stick);
		if(playingTape){
			return currentTape.currentState.getJoystick(stickIndex).buttons[index];
		} else if(recordingTape){
			if(currentTape.currentState.getJoystick(stickIndex).buttons[index] != stick.getRawButton(index)){
				currentTape.addAction(new ActionButton(tapeTick, stickIndex, index, stick.getRawButton(index)));
			}
		}
